...
def main():
    parser = argparse.ArgumentParser(
        description="A script to train and run a Transformer-based translator."
    )
    ...
    parser.add_argument(
        "--compile_model",
        action="store_true",
        help="Pre compile the model with torch.compile",
    )
    parser.add_argument(
        "--epochs", type=int, default=100, help="Number of training epochs."
    )
    parser.add_argument(
        "--max_seq_len", type=int, default=128, help="Maximum length of input data."
    )
    parser.add_argument(
        "--max_dataset_size",
        type=int,
        default=10000000,
        help="Optionally limit dataset size for faster training",
    )
    parser.add_argument(
        "--batch_size",
        type=int,
        default=256,
        help="Effective batch size for training after gradient accumulation",
    )
    parser.add_argument(
        "--gpu_batch_size", type=int, default=64, help="Batch size supported by GPU"
    )
    parser.add_argument(
        "--learning_rate",
        type=float,
    ...
    parser.add_argument(
        ...
        type=str,
        default="data/en.txt",
        help="Path to the English source file.",
    )
    parser.add_argument(
        "--pt_file",
        type=str,
        default="data/pt.txt",
        help="Path to the Portuguese target file.",
    )
    parser.add_argument(
        "--model_path",
        type=str,
    ...
    parser.add_argument(
        "--checkpoint_path",
        type=str,
        default="translator_model_checkpoint.pth",
        help="Path to save the checkpoints during training.",
    )
